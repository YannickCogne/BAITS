...
#### Step2: Fishing in the ocean of species-assemblies ###
	...
	for ind in setofind:
		...
		os.system("blastn  -db "+cwd+"\\blast_index\\"+ind+" -query "+cwd+"\\tmp\\bioM.fasta -outfmt 5 -out "+cwd+"\\tmp\\"+ind+"XbioM.xml" )
...
def makedbblastprot(setofind):
	...
	for elem in setofind:
		...
		if not ((elem+"_prot.phr") in presfile):
			os.system("makeblastdb  -in "+cwd+"\\prot_bdd\\T-"+elem+".fasta -out "+cwd+"\\blast_index\\"+elem+"_prot -dbtype \"prot\"")
		else:
		 ...
		 for elem2 in dico[elem]["peptind"].keys():
			 for ind in dico[elem]["peptind"][elem2]:
				if  dico[elem]["peptind"][elem2][ind]["ident"]<100 or not dico[elem]["peptind"][elem2][ind]["trypok"] :
					...
					os.system("blastp  -db "+cwd+"\\blast_index\\"+ind+"_prot -query "+cwd+"\\tmp\\"+ind+"_"+elem2+"_bioM.fasta -outfmt 5 -out "+cwd+"\\tmp\\"+ind+"_"+elem2+"_bioM.xml" )
					listetmp=set()
